...
return
HandleError<FileHandle>(status, matryoshka::data::errors::ArgumentError()
);
}
...
if (!result) {
return
HandleError<FileHandle>(status, std::move(result)
);
}

return new
FileHandle (std::move(std::get<matryoshka::data::File>(result)));
}
...
Status *Pull(FileSystem * file_system, FileHandle * file,
const char *file_path
) {
if (file_system == nullptr || file == nullptr || file_path == nullptr) {
return new
Status (matryoshka::data::Error(matryoshka::data::errors::ArgumentError()));
}
...
int Find(FileSystem * file_system,
const char *path,
